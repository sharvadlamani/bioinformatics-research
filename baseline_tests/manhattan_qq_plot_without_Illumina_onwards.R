...
data_dir <- "/gpfs/commons/home/svadlamani/skat_all_without_Illumina_onwards"
...
if (nrow(don) == 0) {
  ...
  annotation_threshold <- -log10(3e-5)
  
  # Add annotation labels based on the threshold
  don <- don %>%
    mutate(Label = ifelse(-log10(P1) >= annotation_threshold, Gene, NA))
  ...
  if (!is.null(axisdf)) {
    plot <- ggplot(don, aes(x = BPcum, y = -log10(P1), color = Cell_Type)) +
      ...
      scale_y_continuous(expand = c(0, 0), limits = c(0, 20)) +  # Increase the y-axis limit
      xlab("Chromosome") +
      ylab("-log10(P-value)") +
      theme_bw() +
    ...
    ggsave("manhattan_plot_without_Illumina_onwards.png", plot, dpi = 300, width = 20, height = 10)
  ...
  pdf("QQ_plots_without_Illumina_onwards.pdf")
  
  qq(p_values_burden, main = "Burden QQ Plot")
  qq(p_values_skat, main = "SKAT QQ Plot")
  qq(p_values_skato, main = "SKATO QQ Plot")
