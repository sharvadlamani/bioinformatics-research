...
#Without others (Illumina etc)
X <- XY[c('Sex','Age','apoe_e4','apoe_e2')] # Choose covariates
...
#for(gene in 1:2){
  ...
  count <- count + 1
  print(paste("Gene", count, "over", length(Gs)))
  tryCatch({
    G <- read.csv(Gs[gene])
    S <- read.csv(Gs[gene])
    if (RACE != "ALL") {
    ...
    # # Filter columns where the sum is greater than or equal to 10
    # filtered_data_G <- G[, column_sums >= 10]
...
dir.create(paste0(my_output_path,OUTPUT,"/", TYPE_))
dir.create(paste0(my_output_path,OUTPUT,"/", TYPE_, "/", ITERATION))
write.table(df_p_value, file=paste0(my_output_path,OUTPUT,"/", TYPE_, "/", ITERATION,'/chr',CHRO_NB,'.csv'), quote=FALSE, sep='\t', row.names = FALSE)